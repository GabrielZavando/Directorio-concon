# Change Spec — frontend-spa-navigation

> New capability: the Angular SPA routing layer for the public site. Four skeleton pages (`DirectorioPage`, `EventosPage`, `ContactoPage`, `RegistratePage`) are registered as lazy `loadComponent` routes alongside the existing `/` (HomePage). Each skeleton is a dumb presentational component that renders only its name as a heading and a "Próximamente" message, using the canonical M3 design tokens — no business logic, no DI beyond Router. The placeholder `PlaceholderDirectorioComponent` is replaced by the definitive `DirectorioPageComponent` skeleton (the previously embedded `SearchBarContainerComponent` is removed from the skeleton; search logic lives exclusively in the shared `SearchBarContainerComponent` used by the home hero, and will be re-plugged into the real directory listing in a future change). This spec is the contract for what the routes and the skeleton pages MUST do; visual detail of the active-link styling is covered by the `frontend-layout-base` delta (SPA navigation via routerLink).
>
> Naming convention: skeleton page components use the `-page.component.ts` suffix to align with the pre-existing `home-page.component.ts` pattern in the codebase. Symbols are `DirectorioPageComponent`, `EventosPageComponent`, `ContactoPageComponent`, `RegistratePageComponent`.
