...
#### Scenario: Owner updates place contact fields
- **WHEN** the publisher who owns the place sends `PUT /api/v1/places/{id}` with `telefono`, `email`, `sitioWeb`
- **THEN** response `200` with updated place, `updatedAt` refreshed
...
- **WHEN** a different publisher (different `usuarioId`) sends `PUT`
- **THEN** response `403`

#### Scenario: Admin can update any place
- **WHEN** an admin sends `PUT`
- **THEN** response `200` regardless of `usuarioId`

---

### Requirement: Delete place
The system SHALL allow deletion of a place. Deletion is blocked if any `solicitud` references the place.

#### Scenario: Owner deletes place with no pending solicitudes
- **WHEN** owner sends `DELETE /api/v1/places/{id}` and no `solicitud` has `placeId = {id}`
- **THEN** response `200` with `{ deleted: true, id }`, place document removed

#### Scenario: Delete blocked by existing solicitud
- **WHEN** a `solicitud` with `placeId = {id}` exists (any status)
- **AND** owner or admin sends `DELETE`
- **THEN** response `409` with error "No se puede eliminar: existen solicitudes asociadas a este lugar"
- **AND** place document is NOT removed

---

### Requirement: Solicitud auto-creation on place create
The system SHALL automatically create a `solicitud` document when a place is created.

#### Scenario: Solicitud created with correct linkage
- **WHEN** a place is created via `POST /api/v1/places`
- **THEN** a `solicitud` document exists in `solicitudes` with:
  - `tipo: 'registro'`
  - `status: 'pendiente'`
  - `placeId` = the new place's `id`
  - `usuarioId` = the publisher's UID
