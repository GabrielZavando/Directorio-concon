...
~~This requirement closes a divergence between `docs/data-model/data-model.md` (`usuarioId: "Propietario (Firebase Auth UID)"`) and `docs/api/api-spec.yml:320-321` (which previously listed `usuarioId` as an accepted `CreatePlace` property — a spec artifact that, if ever wired naively by a future `auth` implementation, would allow a client to spoof the `usuarioId` of someone else's place).~~
