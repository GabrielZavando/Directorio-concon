# firebase-credentials-file Specification

## Purpose
The backend loads the Firebase Admin SDK service-account credential from `firebase-admin.json` at
the repository root (or `FIREBASE_ADMIN_CREDENTIALS_PATH`) when present, and auto-enables Firebase
when `FIREBASE_ENABLED` is not `"false"`. `FIREBASE_*` env vars become optional (fallback for
Docker/CI without the file). The credentials file MUST be excluded from version control.
