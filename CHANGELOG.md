...
- **Align `opencode.json` with synced Specboot agents/skills** (chore/specboot-sync follow-up): `agent.plan.prompt` now references `{file:ai-specs/agents/plan-agent.md}` (removes the stale inline prompt that still called the non-existent `openspec new change`); `command.verify` reads scenarios from `.openspec/changes/<change>/`; `command.adversarial-review` description corrected from "8-phase" to the adversarial red-team audit; `instructions[]` trimmed to `[docs/base-standards.md, AGENTS.md]` (conditional loading now handled by `AGENTS.md`). `provider`, `permission`, `build` and `reviewer` blocks untouched; `check-refs.sh` and `specboot.sh --ci` pass with 0 errors.
...
- **Complete Specboot `.opencode/` + `ai-specs` sync** (chore/opencode-minimal): the local project was missing the `.opencode/commands/*` and `.opencode/agents/*` directories that OpenCode auto-loads for the SDD slash-commands and subagents. Synced them from upstream, plus the missing `ai-specs/agents/archive-agent.md` and `verify-agent.md` and `ai-specs/skills/{archive,verify,explain,show-spec-working}`. Registered `explain` and `show-spec-working` in `AGENTS.md`. Removed stale `.opencode/skills/openspec-*` (superseded by `.opencode/commands/`). `check-refs.sh` and `specboot.sh --ci` pass with 0 errors.
